export function ReminderFormComponent(reminder, router) {
    ...
    const textInput = element.querySelector('#text'), timeInput = element.querySelector('#time'), sendBtn = element.querySelector('#sendBtn'), radios = element.querySelectorAll('input[name="category"]');
    flatpickr(timeInput, { enableTime: true, dateFormat: "Y-m-d H:i", defaultDate: isEditing ? new Date(reminder.FullDate) : null, locale: "es" });
    function validate() { const isValid = textInput.value.trim() !== '' && timeInput.value && document.querySelector('input[name="category"]:checked'); sendBtn.disabled = !isValid; }
    textInput.oninput = validate; timeInput.oninput = validate; radios.forEach(r => r.onchange = validate);
    element.querySelector('.back-btn').onclick = () => router.navigate('remindersList');
    sendBtn.onclick = () => {
        const payload = { Type: isEditing ? 'edit_reminder' : 'add_reminder', tracking_code: isEditing ? reminder.tracking_code : null, description: textInput.value.trim(), time: timeInput._flatpickr.selectedDates[0].toISOString(), category: element.querySelector('input[name="category"]:checked').value };
