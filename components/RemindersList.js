export function RemindersListComponent(appData, router) {
    ...
    if (items.length > 0) {
        html += items.map(item => `
            <li class="list-item" id="reminder-${item.tracking_code}">
                <div class="list-item-content">
                    <div class="list-item-details">
                        <span class="list-item-title">${item.Descripcion}</span>
                        <span class="list-item-subtitle">${item.Categoria === 'diario' ? 'Diario a las ' : ''}${item.Fecha}</span>
                    </div>
                    <button class="card-menu-btn" data-menu-id="menu-rem-${item.tracking_code}">⋮</button>
                </div>
                <div class="dropdown-menu" id="menu-rem-${item.tracking_code}">
                    <a class="dropdown-item" data-action="edit-reminder" data-id="${item.tracking_code}">Editar</a>
                    <a class="dropdown-item" data-action="delete-reminder" data-id="${item.tracking_code}">Eliminar</a>
                </div>
            </li>`).join('');
    } else {
    ...
    element.addEventListener('click', (e) => {
        ...
        if (actionItem) {
            ...
            if (actionItem.dataset.action === 'edit-reminder') {
                const reminder = appData.remindersList.find(r => r.tracking_code === id);
                router.navigate('editReminder', { reminder });
            } else if (actionItem.dataset.action === 'delete-reminder') {
                if (confirm('¿Eliminar este recordatorio?')) App.actions.deleteReminder(id);
            }
        }
    });
