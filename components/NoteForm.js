export function NoteFormComponent(note, router) {
    ...
    const titleInput = element.querySelector('#note-title'), descInput = element.querySelector('#note-description'), saveBtn = element.querySelector('#save-note-btn');
    function validate() { const isValid = titleInput.value.trim() !== '' && descInput.value.trim() !== ''; saveBtn.disabled = !isValid; }
    titleInput.oninput = validate; descInput.oninput = validate;
    element.querySelector('.back-btn').onclick = () => router.navigate('notesList');
    saveBtn.onclick = () => {
        const payload = { Type: isEditing ? 'edit_note' : 'add_note', note_id: isEditing ? note.Nota_ID : null, title: titleInput.value.trim(), description: descInput.value.trim() };
